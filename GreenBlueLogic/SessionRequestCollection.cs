...
namespace Ecyware.GreenBlue.Protocols.Http
{
	...
	[Serializable]
	public class SessionRequestCollection : IDictionary, ICloneable
	{
		#region Member Variables
		private const int DEFAULT_CAPACITY = 16;

		private string[] keys;
		private SessionRequest[] values;
		private int count;
		[NonSerialized]
		private int version;
		...
		#region Constructors
		/// <summary>
		///		Initializes a new instance of the <see cref="SessionRequestCollection"/> class that is empty, 
		///		has the default initial capacity and is sorted according to the <see cref="IComparable"/> interface 
		///		implemented by each key added to the <b>SessionRequestCollection</b>.
		/// </summary>
		public SessionRequestCollection()
		{
			keys = new string[DEFAULT_CAPACITY];
			values = new SessionRequest[DEFAULT_CAPACITY];
		...
		///		interface implemented by each key added to the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <param name="capacity">The initial number of elements that the <see cref="SessionRequestCollection"/> can contain.</param>
		/// <exception cref="ArgumentOutOfRangeException"><paramref name="capacity"/> is less than zero.</exception>
		public SessionRequestCollection(int capacity)
		{
			if (capacity < 0)
				throw new ArgumentOutOfRangeException("capacity", capacity, "Initial capacity cannot be less than zero.");

			keys = new string[capacity];
			values = new SessionRequest[capacity];
			comparer = Comparer.Default;
		}
		...
		/// </param>
		public SessionRequestCollection(IComparer comparer) : this()
		{
			if (comparer != null)
		...
		/// <param name="capacity">The initial number of elements that the <see cref="SessionRequestCollection"/> can contain.</param>
		/// <exception cref="ArgumentOutOfRangeException"><paramref name="capacity"/> is less than zero.</exception>
		public SessionRequestCollection(IComparer comparer, int capacity) : this(capacity)
		{
			if (comparer != null)
				this.comparer = comparer;
		}
		...
		/// <summary>
		///		Initializes a new instance of the <see cref="SessionRequestCollection"/> class that contains 
		...
		///		<para>-or-</para>
		///		<para><pararef name="d"/> contains elements of a type not supported by <see cref="SessionRequestCollection"/>.</para>
		///	</exception>
		public SessionRequestCollection(IDictionary d) : this(d, null) {}
		...
		///		<para><pararef name="d"/> contains elements of a type not supported by <see cref="SessionRequestCollection"/>.</para>
		///	</exception>
		public SessionRequestCollection(IDictionary d, IComparer comparer)
		...
		#region Properties
		/// <summary>
		///		Gets and sets the value associated with a specific key in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <value>
		///		The <see cref="SessionRequest"/> associated with <paramref name="key"/> in the <see cref="SessionRequestCollection"/>,
		...
		///		Callers must either catch this exception, or call the <see cref="SessionRequestCollection.Contains"/> method first to
		///		see if the key exists.
		/// </remarks>
		/// <exception cref="ArgumentNullException">The <paramref name="key"/> is a null reference.</exception>
		/// <exception cref="ArgumentException">
		///		Either the <paramref name="key"/> or the <paramref name="value"/> are not of a type supported by the <see cref="SessionRequestCollection"/>.
		/// </exception>
		/// <exception cref="NullReferenceException">
		///		<paramref name="key"/> is not found in the <see cref="SessionRequestCollection"/>.
		/// </exception>
		object IDictionary.this[object key]
		{
			...
			set
			{
				...
				if (!(value is SessionRequest))
					throw new ArgumentException("The value must be of type: " + typeof(SessionRequest).FullName, "value");

				this[(string)key] = (SessionRequest)value;
			}
		}
		...
		///		<paramref name="key"/> is not found in the <see cref="SessionRequestCollection"/>.
		/// </exception>
		public virtual SessionRequest this[string key]
		{
			get
			{
		...
		/// <summary>
		///		Gets or sets the capacity of the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <value>The number of elements that the <see cref="SessionRequestCollection"/> can contain.</value>
			...
			set
			{
				...
				if (value != keys.Length)
				{
					if (value > 0)
					{
						...
						SessionRequest[] newValues = new SessionRequest[value];
		...
		/// <summary>
		///		Gets the number of elements contained in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <value>The number of elements contained in the <see cref="SessionRequestCollection"/>.</value>
		...
		/// <summary>
		///		Gets a value indicating whether the <b>SessionRequestCollection</b> has a fixed size.
		/// </summary>
		/// <value>
		///		<b>true</b> if the <see cref="SessionRequestCollection"/> has a fixed size; otherwise, <b>false</b>.
		...
		/// <summary>
		///		Gets a value indicating whether the <b>SessionRequestCollection</b> is read-only.
		/// </summary>
		/// <value>
		///		<b>true</b> if the <see cref="SessionRequestCollection"/> is read-only; otherwise, <b>false</b>.
		///		The default is <b>false</b>.
		/// </value>
		...
		/// <summary>
		///		Gets a value indicating whether access to the <b>SessionRequestCollection</b> is
		///		synchronized (thread-safe).
		/// </summary>
		/// <value>
		///		<b>true</b> if access to the the <see cref="SessionRequestCollection"/> is 
		...
		/// <summary>
		///		Gets the keys in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <value>An <see cref="ICollection"/> containing the keys in the <see cref="SessionRequestCollection"/>.</value>
		/// <remarks>
		///		<para>The <see cref="ICollection"/> is a read-only view of the keys in the 
		///		<see cref="SessionRequestCollection"/>. Modifications made to the underlying <b>SessionRequestCollection</b>
		...
		///		of the <b>SessionRequestCollection</b>.</para>
		///		<para>Similar to <see cref="SessionRequestCollection.GetKeyList"/>, but returns an <b>ICollection</b>
		...
		/// <summary>
		///		Gets an object that can be used to synchronize access to the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <value>An object that can be used to synchronize access to the <see cref="SessionRequestCollection"/>.</value>
		...
		/// <summary>
		///		Gets the values in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <value>An <see cref="ICollection"/> containing the values in the <see cref="SessionRequestCollection"/>.</value>
		/// <remarks>
		...
		///		of the <b>SessionRequestCollection</b>.</para>
		///		<para>Similar to <see cref="SessionRequestCollection.GetValueList"/>, but returns an <b>ICollection</b>
		///		instead of an <see cref="IList"/>.</para>
		/// </remarks>
		...
		///		<para>-or-</para>
		///		<para>Either the <paramref name="key"/> or the <paramref name="value"/> are not of a type supported by the <see cref="SessionRequestCollection"/>.</para>
		///		<para>-or-</para>
		...
		void IDictionary.Add(object key, object value)
		{
			...
			this.Add((string)key, (SessionRequest)value);
		}
		...
		/// <summary>
		///		Removes all elements from the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <exception cref="NotSupportedException">
		///		<para>The <see cref="SessionRequestCollection"/> is read-only.</para>
		///		<para>-or-</para>
		...
		/// <summary>
		///		Creates a shallow copy of the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <returns>A shallow copy of the <see cref="SessionRequestCollection"/>.</returns>
		public virtual object Clone()
		{
			SessionRequestCollection newList = new SessionRequestCollection(count);
		...
		/// <summary>
		///		Determines whether the <b>SessionRequestCollection</b> contains a specific key.
		/// </summary>
		/// <param name="key">The key to locate in the <see cref="SessionRequestCollection"/>.</param>
		/// <exception cref="ArgumentNullException">The <paramref name="key"/> is a null reference.</exception>
		/// <exception cref="ArgumentException">
		///		The <paramref name="key"/> is not of a type supported by the <see cref="SessionRequestCollection"/>.
		/// </exception>
		/// <exception cref="InvalidOperationException">The comparer throws an exception.</exception>
		/// <returns>
		///		<b>true</b> if the <see cref="SessionRequestCollection"/> contains an element with the specified key; 
		///		otherwise, <b>false</b>.
		///	</returns>
		...
		/// <summary>
		///		Copies the <see cref="SessionRequestCollection"/> elements to a one-dimensional <see cref="System.Array"/>
		///		instance at the specified index.
		/// </summary>
		/// <param name="array">
		///		The one-dimensional <see cref="System.Array"/> that is the destination of the
		///		<see cref="DictionaryEntry"/> objects copied from <see cref="SessionRequestCollection"/>.
		...
		///		<para>-or-</para>
		///		<para>The number of elements in the source <see cref="SessionRequestCollection"/> is greater than the 
		///		available space from <paramref name="arrayIndex"/> to the end of the destination <paramref name="array"/>.</para>
		/// </exception>
		/// <exception cref="InvalidCastException">
		///		The type of the source <see cref="SessionRequestCollection"/> cannot be cast automatically to the type
		...
		/// <summary>
		///		Gets the value at the specified index of the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <param name="index">The zero-based index of the value to get.</param>
		/// <returns>The value at the specified index of the <see cref="SessionRequestCollection"/>.</returns>
		/// <exception cref="ArgumentOutOfRangeException">
		///		<paramref name="index"/> is outside the range of valid indices for the <see cref="SessionRequestCollection"/>.
		/// </exception>
		public virtual SessionRequest GetByIndex(int index)
		{
		...
		/// <summary>
		///		Returns an <see cref="IEnumerator"/> that can iterate through the <b>SessionRequestCollection</b>.
		/// </summary>
		...
		///		<b>SessionRequestCollection</b>.		
		/// </summary>
		/// <returns>An <see cref="IDictionaryEnumerator"/> for the <see cref="SessionRequestCollection"/>.</returns>
		...
		/// <summary>
		///		Gets the key at the specified index of the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <param name="index">The zero-based index of the key to get.</param>
		/// <returns>The key at the specified index of the <see cref="SessionRequestCollection"/>.</returns>
		/// <exception cref="ArgumentOutOfRangeException">
		///		<paramref name="index"/> is outside the range of valid indices for the <see cref="SessionRequestCollection"/>.
		/// </exception>
		...
		///		Gets the keys in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <returns>An <see cref="IList"/> containing the keys in the <see cref="SessionRequestCollection"/>.</returns>
		/// <remarks>
		...
		///		of the <b>SessionRequestCollection</b>.</para>
		///		<para>Similar to <see cref="SessionRequestCollection.Keys"/>, but returns an <b>IList</b>
		...
		///		Gets the values in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <returns>An <see cref="IList"/> containing the values in the <see cref="SessionRequestCollection"/>.</returns>
		/// <remarks>
		...
		///		of the <b>SessionRequestCollection</b>.</para>
		///		<para>Similar to <see cref="SessionRequestCollection.Values"/>, but returns an <b>IList</b>
		///		instead of an <see cref="ICollection"/>.</para>
		/// </remarks>
		...
		/// <summary>
		///		Returns the zero-based index of the specified key in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <param name="key">The key to locate in the <see cref="SessionRequestCollection"/>.</param>
		/// <returns>
		...
		/// <exception cref="InvalidOperationException">The comparer throws an exception.</exception>
		/// <remarks>
		///		<para>The elements of a <see cref="SessionRequestCollection"/> are sorted by the keys either 
		///		according to a specific <see cref="IComparer"/> implementation specified when the 
		///		<see cref="SessionRequestCollection"/> is created or according to the <see cref="IComparable"/> 
		...
		///		it is inserted into <see cref="SessionRequestCollection"/> in the correct sort order, and the 
		...
		///		removed from the <see cref="SessionRequestCollection"/>.</para>
		///		<para>This method uses a binary search algorithm; therefore, the average execution time is 
		///		proportional to Log2(<i>n</i>), where <i>n</i> is <see cref="SessionRequestCollection.Count"/>.</para>
		/// </remarks>
		...
		/// <summary>
		///		Removes the element with the specified key from the <b>SessionRequestCollection</b>.
		/// </summary>
		...
		/// </exception>
		/// <remarks>
		///		If the <see cref="SessionRequestCollection"/> does not contain an element with the specified key,
		///		the <b>SessionRequestCollection</b> remains unchanged. No exception is thrown.
		/// </remarks>
		...
		/// <summary>
		///		Returns a synchronized (thread-safe) wrapper for the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <param name="list">The <see cref="SessionRequestCollection"/> to synchronize.</param>
		/// <returns>A synchronized (thread-safe) wrapper for the <see cref="SessionRequestCollection"/>.</returns>
		/// <exception cref="ArgumentNullException"><paramref name="list"/> is a null reference.</exception>
		public static SessionRequestCollection Synchronized(SessionRequestCollection list)
		{
			if (list == null)
		...
		/// <summary>
		///		Sets the capacity to the actual number of elements in the <b>SessionRequestCollection</b>.
		/// </summary>
		/// <exception cref="NotSupportedException">
		///		<para>The <see cref="SessionRequestCollection"/> is read-only.</para>
		///		<para>-or-</para>
		...
		[Serializable]
			private class SyncSortedList : SessionRequestCollection, IDictionary
		{
			private SessionRequestCollection list;
			private object root;

			internal SyncSortedList(SessionRequestCollection list)
			{
				this.list = list;
			...
			public override SessionRequest this[string key]
			{
				get
				{
					lock (root)
			...
			public override SessionRequest GetByIndex(int index)
			{
				lock (root)
			...
			private class SortedListEnumerator : IDictionaryEnumerator, ICloneable
		{
			...
			internal SortedListEnumerator(SessionRequestCollection list, int index, int count, int returnType)
			{
				this.list = list;
				this.index = index;
			...
			public virtual SessionRequest Value
			{
				get
				{
					CheckState();
			...
			private class KeyList : IList
		{
			...
			internal KeyList(SessionRequestCollection list)
			{
				this.list = list;
			}
			...
			private class ValueList : IList
		{
			...
			internal ValueList(SessionRequestCollection list)
			{
				this.list = list;
			}
			...
			bool IList.Contains(object value)
			{
				...
				return list.ContainsValue((SessionRequest)value);
			}
			...
			object IList.this[int index]
			{
				...
				set
				{
					...
					list.SetByIndex(index, (SessionRequest)value);
				}
			}
			...
			int IList.IndexOf(object value)
			{
				...
				return list.IndexOfValue((SessionRequest)value);
			}
